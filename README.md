...
| مورد | مدل | کارش |
|---|---|---|
| **غیراخلاقی** | Marqo ViT-tiny + MobileNetV4 | به عکس و گیف و استیکر و کاور ویدیو نمره می ده؛ بالاتر از حساسیت گروه پاک می شه |
| **موضوعی** | CLIP ViT-B/32 | سیگار · مشروب · اسلحه · قمار · مواد · خون |
...
مدل موضوعی (`clip.onnx`، ۳۳۵ مگ) تو ریپو نیست. بقیه مدل ها و خود ONNX Runtime با
`include_bytes!` تو باینری ان، ولی این یکی انقدر بزرگه که rustc موقع لینک چند گیگ رم می خواد،
واسه همین از رو دیسک و از **کنار فایل اجرایی** لود می شه. اگه قفل موضوعی رو می خوای:

```sh
curl -L -o target/release/clip.onnx \
  https://huggingface.co/Xenova/clip-vit-base-patch32/resolve/main/onnx/vision_model.onnx
```

نباشه فقط قفل های موضوعی کار نمی کنن، یه خط تو لاگ می گه و بقیه ربات سر جاشه.
