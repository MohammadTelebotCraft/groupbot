...
async fn main() -> Result {
    ...
    if migrating {
        ...
        for (chat, name) in &examples {
            ...
            settings
                .set(*chat, &format!("{}{name}", handlers::imgfilter::PREFIX), false)
                .await;
            println!("vision: retired example filter «{name}» in {chat}; its pictures are from the old space");
        }
    ...
    if migrating && settings.set(0, &space_flag, true).await {
        println!("vision: database vectors are now from {}", handlers::vision::CHECKPOINT);
    }
