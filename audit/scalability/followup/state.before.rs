...
const INDEXES: &[&str] = &[
    ...
    "CREATE INDEX IF NOT EXISTS moderation_case_events_case ON moderation_case_events (case_id, id)",
];
...
impl Settings {
    ...
    async fn connect_inner(
        ...
        let size = std::env::var("DB_POOL")
            ...
            .unwrap_or(8)
            .clamp(2, 64);
        let statement_ms = crate::configured_cap("DB_STATEMENT_TIMEOUT_MS", 5_000, 100, 120_000);
        let lock_ms = crate::configured_cap("DB_LOCK_TIMEOUT_MS", 1_000, 50, 30_000).min(statement_ms);
        ...
        let migration_ms = crate::configured_cap("DB_MIGRATION_TIMEOUT_MS", 120_000, 1_000, 3_600_000);
        let migration_lock_ms = crate::configured_cap("DB_MIGRATION_LOCK_TIMEOUT_MS", 30_000, 100, 120_000).min(migration_ms);
        sqlx::query("SELECT set_config('statement_timeout', $1, false), set_config('lock_timeout', $2, false)")
    ...
    pub async fn cleaner_recommendations_due(&self, minutes: &[i64]) -> Vec<i64> {
        let rows: Vec<(i64,)> = sqlx::query_as(
            "SELECT chat_id FROM settings
             WHERE key = 'hash' AND chat_id < 0 AND value <> ''
        ...
        .unwrap_or_else(|error| {
            log::warn!("settings: cleaner recommendation query failed: {error}");
            Vec::new()
        });
        rows.into_iter().map(|(chat,)| chat).collect()
    }

    pub async fn incomplete_setups(&self, include_cleaner: bool) -> Vec<i64> {
        let rows: Vec<(i64,)> = sqlx::query_as(
        ...
        .await
        .unwrap_or_else(|error| {
            log::warn!("settings: incomplete setups lookup failed: {error}");
            Vec::new()
        });
        rows.into_iter().map(|(chat,)| chat).collect()
    }
    ...
    pub async fn create_moderation_case(&self, draft: NewModerationCase) -> Result<i64> {
        ...
        sqlx::query(
            "INSERT INTO moderation_case_events
             (case_id, kind, actor_id, actor_name, action, note, created_at)
    ...
    pub async fn moderation_case(
        ...
        let events = sqlx::query(
            "SELECT id, case_id, kind, actor_id, actor_name, action, note, created_at
             FROM moderation_case_events WHERE case_id = $1 ORDER BY id",
        )
        .bind(id)
        .fetch_all(&self.pool)
        .await?
    ...
    pub async fn add_moderation_case_note(
        ...
        let result = sqlx::query(
            "INSERT INTO moderation_case_events
             (case_id, kind, actor_id, actor_name, note, created_at)
             SELECT id, 'note', $3, $4, $5, $6 FROM moderation_cases
    ...
    pub async fn append_moderation_case_event(
        ...
        let result = sqlx::query(
            "INSERT INTO moderation_case_events
             (case_id, kind, actor_id, actor_name, action, note, created_at)
             SELECT id, $3, $4, $5, $6, $7, $8 FROM moderation_cases
             WHERE chat_id = $1 AND id = $2",
        )
        .bind(chat)
        .bind(id)
    ...
    pub async fn purge_expired_moderation_cases(&self) -> Result<u64> {
        ...
        let result = sqlx::query(
            "DELETE FROM moderation_cases WHERE id IN
             (SELECT id FROM moderation_cases WHERE created_at < $1 ORDER BY created_at, id LIMIT 10000)",
        )
        .bind(cutoff)
        .execute(&self.pool)
        .await?;
...
mod tests {
    ...
    async fn cleaner_recommendations_select_only_due_known_groups() {
        ...
        sqlx::query(wipe)
            ...
            .await
            .unwrap();
        settings.set_value(chats[0], "hash", "123").await;
        settings.set_value(chats[1], "hash", "456").await;
        settings.set_value(chats[2], "owner", "42").await;
        settings.set_value(chats[3], "hash", "789").await;
        settings.set(chats[0], "cln_added", true).await;
        let pending = settings.incomplete_setups(false).await;
        assert!(pending.contains(&chats[0]));
        assert!(pending.contains(&chats[1]));
        assert!(!pending.contains(&chats[2]));
        assert!(!pending.contains(&chats[3]));
        settings.set_value(chats[0], "owner", "42").await;
        assert!(!settings.incomplete_setups(false).await.contains(&chats[0]));
        settings.set_value(chats[1], "owner", "42").await;
        assert!(!settings.incomplete_setups(false).await.contains(&chats[1]));
        let pending = settings.incomplete_setups(true).await;
        assert!(
        ...
        let due = settings.cleaner_recommendations_due(&[bucket]).await;
        assert!(due.contains(&chats[0]));
        assert!(!due.contains(&chats[1]));
        assert!(!due.contains(&chats[3]));
        let all = settings
            .cleaner_recommendations_due(&(0..360).collect::<Vec<_>>())
            .await;
        assert!(!all.contains(&chats[2]));
        assert!(!all.contains(&chats[3]));
        assert!(settings.cleaner_recommendations_due(&[]).await.is_empty());
        settings.set(chats[0], "hash", false).await;
        assert!(
            !settings
    ...
    async fn moderation_cases_are_isolated_audited_and_expire() {
        ...
        sqlx::query("UPDATE moderation_cases SET created_at = $1 WHERE id = $2")
        ...
        let events: i64 =
            sqlx::query_scalar("SELECT count(*) FROM moderation_case_events WHERE case_id = $1")
                .bind(id)
                .fetch_one(&settings.pool)
                .await
                .unwrap();
